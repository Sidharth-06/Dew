...
final jamSessionData = ValueNotifier<Map<String, dynamic>?>(null);
...
Future<List> getRecommendedSongs(String query) async {
  try {
...
Future<String?> fetchLyricsFromGenius(String artist, String title) async {
  const geniusAccessToken =
      '8fZjCafgJWMNo0NWauLQnIIdRNbsz1S1Wu4WqCjZCHCON606UMMYAiCql_kLub43'; // Replace with your Genius API token
  final genius = Genius(accessToken: geniusAccessToken);

  try {
    // Search for the song
    final song = await genius.searchSong(artist: artist, title: title);

    // Return the lyrics if found
    if (song != null) {
      return song.lyrics;
    } else {
    ...
    debugPrint('Error fetching lyrics from Genius: $e');
    return null;
  }
}
...
Future<List<String>> getSearchSuggestions(String query,
    {required int playlistsNum}) async {
...
Future<Map> getariyillaSongs() async {
  const playlistId = 'PLgzTt0k8mXzEk586ze4BjvDXR7c-TUSnx';
  List songList = await getSongsFromPlaylist(playlistId);
...
Future<String> getSong(String songId, bool isLive) async {
  try {
    ...
    final results = await Future.wait([
      ...
      updateRecentlyPlayed(songId), // No need to await this
    ]);

    final cachedUrl = results[0] as String?;
...
/// Removes common extraneous parts from a YouTube song title.
String cleanSongTitle(String title) {
  // Remove content inside parentheses (e.g., "(Official Video)", "(Lyrics)")
  final regex = RegExp(r'\s*\(.*?\)\s*');
  String cleaned = title.replaceAll(regex, '');

  // Remove trailing parts after a dash if they appear to be extra (e.g., " - Official Video")
  if (cleaned.contains(' - ')) {
...
Future<String?> getSongLyrics(String artist, String title) async {
  const geniusAccessToken =
      '8fZjCafgJWMNo0NWauLQnIIdRNbsz1S1Wu4WqCjZCHCON606UMMYAiCql_kLub43'; // Your Genius API token
  final genius = Genius(accessToken: geniusAccessToken);

  // Clean the song title so it matches Genius data better
  final cleanedTitle = cleanSongTitle(title);

  try {
    // Search for the song using Genius API.
    // For example, if your YouTube metadata title is "No Love (Official Video) - Eminem",
    // cleaning it would yield "No Love"
    final song = await genius.searchSong(artist: artist, title: cleanedTitle);
